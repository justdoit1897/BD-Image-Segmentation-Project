...
def split_rle(rle_string: str) -> tuple[list, list]:
    """Funzione per identificare, in una stringa di codifica RLE,
    i pixel iniziali di ogni run e la lunghezza della run

    Args:
        rle_string (str): stringa contenente la codifica RLE da separare

    Returns:
        tuple[list, list]: liste dei pixel di inizio run e corrispettive
        lunghezze delle run
    """
...
def coordinate_pixel(posizioni: list, larghezza: int) -> tuple[list, list]:
    """Funzione usata per estrarre le coordinate dei pixel in cui iniziano le run
    di una codifica RLE.
    
    Args:
        posizioni (list): lista di posizioni di inizio estratte da una codifica RLE
        larghezza (int): larghezza desiderata dell'immagine
    
    Returns:
        tuple[list, list]: liste di coordinate x, e corrispettive y, dei pixel
        di inizio delle run di una codifica RLE
...
def crea_maschera_vuota(width: int, height: int, pixel_size: list) -> np.ndarray:
    """Funzione per generare una maschera vuota

    Args:
        width (int): larghezza (in pixel) desiderata per l'immagine
        height (int): altezza (in pixel) desiderata per l'immagine
        pixel_size (list): lista delle dimensioni di un pixel (in larghezza e altezza)

    Returns:
        np.ndarray: maschera vuota
    """
...
def crea_maschera(width: int, height: int) -> np.ndarray:
    """Funzione per creare una maschera vuota, indipendente dalla dimensione dei pixel
...
def riempi_maschera(mask: np.ndarray, x_coords: list, y_coords: list, color: int, lengths=None) -> np.ndarray:
    """Riempie una maschera con i punti dati dalle coordinate con un colore specifico e una lunghezza specifica.
    
        Args:
            mask (np.ndarray): maschera da sovrascrivere con il riempimento
            x_coords (list): lista di ascisse di inizio run
            y_coords (list): lista di ordinate di inizio run
            color (int): colore desiderato per il riempimento
            lengths: lista di lunghezze per le run
        
        Returns:
            np.ndarray: maschera colorata correttamente
    """
    if lengths is None:
